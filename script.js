...
function calculateAndDisplayRoute(directionsService, directionsRenderer, end, start, numOfStops, map) {
  ...
  var attractions = [];
  var newLat = (takeLocation(start).lat + takeLocation(end).lat)/2
  var newLong = (takeLocation(start).lng + takeLocation(end).lng)/2
  var midpoint = new google.maps.LatLng(newLat, newLong);
  var r = 10000;
